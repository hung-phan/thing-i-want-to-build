...
/**
  * This is an event sourced entity. It has a state, [[TwitterState]], which
...
class TwitterEntity extends PersistentEntity {
  ...
  override type State = TwitterState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: TwitterState = TwitterState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case TwitterState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        TwitterState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class TwitterState(message: String, timestamp: String)
...
object TwitterState {
  /**
    ...
    */
  implicit val format: Format[TwitterState] = Json.format
}

/**
  * This interface defines all the events that the TwitterEntity supports.
  */
sealed trait TwitterEvent extends AggregateEvent[TwitterEvent] {
  def aggregateTag = TwitterEvent.Tag
}

object TwitterEvent {
  val Tag = AggregateEventTag[TwitterEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends TwitterEvent
...
/**
  ...
  */
sealed trait TwitterCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends TwitterCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends TwitterCommand[String]
